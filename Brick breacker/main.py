    ...
    for row in range(rows):
        for col in range(cols):
            brick = Brick(col * brick_width + gap * col,
             row * brick_height + gap * row, brick_width, brick_height, 2, [(255,0,255), (0,0,255)] )
            bricks.append(brick)
...
def main():
    ...
    paddle = Paddle(center_x - PADDLE_WIDTH/2, paddle_y, PADDLE_WIDTH, PADDLE_HEIGHT, "black")
    ...
    def reset():
        paddle.x = center_x - PADDLE_WIDTH/2
        paddle.y = paddle_y
        ball.x = WIDTH/2
